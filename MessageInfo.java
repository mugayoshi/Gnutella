...
public class MessageInfo {
	...
	public MessageInfo(byte[] idIn, byte ttl, byte hop){//for Ping
		this.ID = Arrays.copyOf(idIn, idIn.length);
		this.TTL = ttl;
		this.HOP = hop;
	}
	public MessageInfo(byte[] idIn, byte ttl, byte hop, String searchWord){//for query
		this.ID = Arrays.copyOf(idIn, idIn.length);
		this.TTL = ttl;
		this.HOP = hop;
		this.searchCriteria = searchWord;
	}
	public MessageInfo(ArrayList<byte[]> in ,byte[] Idin, byte ttl, byte hop, String searchWord){
	...
	public MessageInfo(ArrayList<byte[]> in, byte[] idIn, byte ttl, byte hop){//for pong
		this.list = new ArrayList<byte[]>();
		//this.list = in;
		for(int i = 0; i < in.size(); i++){
			byte[] b = Arrays.copyOf(in.get(i), in.get(i).length );
	...
	public MessageInfo(ArrayList<byte[]> in, byte[] idIn, byte ttl, byte hop, byte numHits){//for query hit
		this.list = new ArrayList<byte[]>();
		//this.list = in;
		for(int i = 0; i < in.size(); i++){
			byte[] b = Arrays.copyOf(in.get(i), in.get(i).length );
