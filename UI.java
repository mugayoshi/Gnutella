			...
			try{
				...
				if(this.command.toUpperCase().equals("PING")){
					if(argument.length != 1){
						...
						System.out.println("the format is 'PING' ");
						continue;
					}
					...
					if(argument.length != 2){
						System.out.println("Command Format is Wrong !!");
						System.out.println("the format is 'QUERY <Search Criteria>' ");
						continue;
					}
					...
					if(argument.length != 3){
						System.out.println("Command Format is Wrong !!");
						System.out.println("the format is ' CONNECT <IP Address> <Port No.>' ");
						continue;
					}
					try{
					...
					//	System.out.println("Log: Send Gnutella Connect Message");
						...
						while((line = inputstream.readLine()) != null){
							if(line.contains("GNUTELLA")){
								...
								recevoir.start();
								System.out.println("UI: Recv Thread Starts");
								//
								break;
							}
						}
					...
					if(argument.length != 1){
						System.out.println("Command Format is Wrong !!");
						System.out.println("the format is 'SHOW' or 'HIST' ");
						continue;
					}
					...
					if(argument.length != 1){
						System.out.println("Command Format is Wrong !!");
						System.out.println("the format is 'INFO' ");
						continue;
					}
					MySingleton.showInfo();
					...
					if(argument.length != 1){
						System.out.println("Command Format is Wrong !!");
						System.out.println("the format is 'DOWNLOAD' ");
						continue;
					}
					...
					try{
						...
						System.out.println("Log: Download Dest Port = " + portNum);
						Socket destSocket = new Socket(ip, portNum);
						if(MySingleton.checkSocket(destSocket) >= 0){
							System.out.println("This Destinaton is already connected with myself");
							continue;
						}else{
							MySingleton.addSocket(destSocket);
							OutputStream outstream = destSocket.getOutputStream();
						//	BufferedReader inputstream = new BufferedReader(new InputStreamReader(destSocket.getInputStream()));
							...
							System.out.println("Log: Send HTTP Connect Message");
